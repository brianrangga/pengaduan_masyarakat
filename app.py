...
def like_report(report_id):
    if 'user_id' not in session:
        return jsonify({'success': False, 'error': 'Silakan login untuk memberikan like'}), 401
    ...
    try:
        ...
        if cur.fetchone():
            cur.close()
            return jsonify({'success': False, 'error': 'Anda sudah memberikan like'}), 400

        cur.execute(
            "INSERT INTO report_votes (user_id, report_id, created_at) VALUES (%s, %s, %s)",
            (session['user_id'], report_id, datetime.now())
        )
        cur.execute(
            "UPDATE reports SET voting = voting + 1 WHERE id = %s",
            (report_id,)
        )
        cur.execute("SELECT voting FROM reports WHERE id = %s", (report_id,))
        new_vote_count = cur.fetchone()[0]
        mysql.connection.commit()
        cur.close()
        return jsonify({'success': True, 'new_vote_count': new_vote_count}), 200
    except Exception as e:
        mysql.connection.rollback()
        cur.close()
...
def simpan_tanggapan():
    ...
    comment = request.form.get('comment')  # Ambil komentar dari form
    ...
    try:
        ...
        cur.execute("SELECT id FROM responses WHERE report_id = %s ORDER BY created_at DESC LIMIT 1", (report_id,))
        ...
        if existing_response:
            ...
            cur.execute(
                """
                UPDATE responses
                SET response_status = %s, updated_at = %s
                WHERE id = %s
                """,
                (status, current_time, response_id)
            )
        else:
            # Insert new response
            cur.execute(
        ...
        # Insert into response_progress to track the status change
        cur.execute(
            """
            INSERT INTO response_progress (response_id, status, comment, changed_at, changed_by)
            VALUES (%s, %s, %s, %s, %s)
            """,
            (response_id, status, comment if comment else None, current_time, petugas_id)
        )

        # Update the report's statement to match the latest status
        cur.execute(
            """
            UPDATE reports
            SET statement = %s, updated_at = %s
            WHERE id = %s
            """,
            (status, current_time, report_id)
        )
